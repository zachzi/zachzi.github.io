...
AuthSession.prototype = {
    ...
    getStatus: function () {
                ...
                for (var i = 0; i < scopes.length; i++) {
                    ...
                    if (result)
                    {
                        ...
                        var rawResult = result[1].split("_"),
                            ...
                            selectionId = rawResult[1];

                        ownerCid = selectionId.split("!")[0];
                        resourceId = [selectionString, ownerCid, selectionId].join(".");
                        authKey = result[2];
                    }
                    else {
        ...
        return { status: status, session: session, resource_id: resourceId, owner_cid: ownerCid, authentication_key: authKey };
    },
        ...
        _onExternalConsentComplete: function(response) {
            ...
            var getItemProperties = {
                path: generateSharingLinks ?
                    "drives/" + owneCid + "/items/" + resourceId + "?$expand=children($expand=thumbnails)&authkey=" + authKey : resourceId + "/files",
                method: HTTP_METHOD_GET,
