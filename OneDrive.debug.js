        ...
        internalApp.fileDialog(fileDialogProperties).then(
            ...
            function (fileDialogResponse) {
                ...
                var responseFiles,
                    file,
                    thumbnails,
                    response = fileDialogResponse.apiResponse,
                    files = {
                        link: null,
                        values: []
                    };
                ...
                switch (linkType) {
                    case ONEDRIVE_PARAM_LINKTYPE_DOWNLOAD:
                        ...
                        for (var i = 0; i < responseFiles.length; i++) {
                            ...
                            if (file.images) {
                                thumbnails = file.images.map(function(image) {
                                    return image.source;
                                });
                            }
                    ...
                    case ONEDRIVE_PARAM_LINKTYPE_WEBVIEW:                  
                        responseFiles = response.children;

                        // Sharing view link for the entire bundle. We don't yet have an equivalent for download.
                        files.webViewLink = response.webUrl;

                        for (var j = 0; j < responseFiles.length; j++) {
                            file = responseFiles[j];
                            thumbnails = [];

                            if (file.thumbnails[0]) {
                                thumbnails = OneDriveApp.vroomThumbnailSizes.map(function(size) {
                                    return (file.thumbnails[0][size]).url;
                                });
                            }
                        ...
                        break;  
                    default:
                        throw new Error(ERROR_DESC_LINKTYPE_NOTIMPLEMENTED.replace(ONEDRIVE_PARAM_LINKTYPE, linkType));
                }
                ...
                switch (uploadType) {
                    case UPLOADTYPE_URL:
                        var urlUploadProperties = {
                            ...
                            vroomApi: true,
                            request_headers: [{ name: API_PARAM_PREFER, value: API_PARAM_RESPOND_ASYNC }, { name: API_PARAM_AUTH, value: "bearer " + pickerResponse.authentication_key }],
...
var APIRequest = function (properties) {
    ...
    var path = properties[API_PARAM_PATH],
        vroomApi = properties[API_PARAM_VROOMAPI];
    request._url = isPathFullUrl(path) ?
        path :
        getApiServiceUrl(vroomApi) + (path.charAt(0) === "/" ? path.substring(1) : path);
...
AuthSession.prototype = {
    ...
    getStatus: function () {
        ...
        return { status: status, session: session, resource_id: resourceId, owner_cid: ownerDCid, authentication_key: authKey };
    },
 ...
 */
function getApiServiceUrl(vroomApi) {
    return vroomApi ? wl_app[ONEDRIVE_API] : wl_app[WL_APISERVICE_URI];
}
...
(function () { 
    ...
    FilePickerOperation.prototype = {
        ...
        _onExternalConsentComplete: function(response) {
            ...
            var ownerDCid = response[AK_OWNER_CID],
            ...
            var getItemProperties = {
                path: generateSharingLinks ?
                    "drives/" + ownerDCid + "/items/" + resourceId + "?$expand=children($expand=thumbnails)" : resourceId + "/files",
                method: HTTP_METHOD_GET,
                vroomApi: generateSharingLinks,
                request_headers: generateSharingLinks ? [{ name: API_PARAM_AUTH, value: "bearer " + authKey }] : [],
...
wl_app._locale = "en";
        ...
        wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.8528.2003");
        ...
        prodSettings[ONEDRIVE_API] = "https://api.onedrive.com/v1.0/";
