...
OneDriveApp.prototype = {
    ...
    executeSaveOperation: function () {
        ...
        internalApp.fileDialog(fileDialogProperties).then(
            // Success callback.
            function (fileDialogResponse) {
                var folderId = fileDialogResponse.data[0].id;

                switch (uploadType) {
                    case UPLOADTYPE_URL:
                        {
                            var urlUploadProperties = {
                                path: "me/items/" + folderId + "/children",
                                method: HTTP_METHOD_POST,
                                interface_method: method,
                                vroomApi: true,
                                request_headers: [{ name: API_PARAM_PREFER, value: API_PARAM_RESPOND_ASYNC }],
                            ...
                            internalApp.api(urlUploadProperties).then(
                                ...
                                function (urlUploadResponse) {
                                    ...
                                    (urlUploadResponse[API_PARAM_STATUS_HTTP] === API_STATUS_HTTP_ACCEPTED && !stringIsNullOrEmpty(location)) ?
                                        ...
                                        that.processErrorCallback(urlUploadResponse, ERROR_DESC_OPERATION_UPLOAD_SERVERERROR);
                                },
                                // Error callback.
            ...
            that._internalApp.api(progressApiProperties).then(
                ...
                function(apiResponse) {
                    switch (apiResponse[API_PARAM_STATUS_HTTP]) {
                        ...
                        default:
                            that.processErrorCallback(apiResponse, ERROR_DESC_OPERATION_UPLOAD_SERVERERROR);
                    }
                },
                // Error callback.
                function(apiError) {
...
function stringFormat() {
    ...
    function replaceFunc(match) {
        var replacement = args[parseInt(match.replace(/[\{\}]/g, "")) + 1];
        if (replacement == null) {
            replacement = '';
        }
        return replacement;
...
function deserializeJSON(text) {
    return window.JSON ? JSON.parse(text) : eval("(" + text + ")");
}
...
var ONEDRIVE_PARAM_CANCEL = "cancel",
    ...
    ONEDRIVE_PARAM_LINKTYPE = "linkType",
...
function buildSkyDrivePickerControlStyle(properties, button)
{
    ...
    buttonStyle.paddingLeft = "4px";
    buttonStyle.paddingRight = "4px";
    ...
    textStyle.marginLeft = rtl ? "0px" : "2px";
    textStyle.marginRight = rtl ? "2px" : "0px";
}
...
wl_app._locale = "en";
        ...
        wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.8230.2003");
        ...
        prodSettings[ONEDRIVE_API] = "https://newapi.users.storage.live.com/";
        ...
        dfSettings[ONEDRIVE_API] = "https://newapi-df.users.storage.live.com/";
        ...
        intSettings[ONEDRIVE_API] = "https://newapi.users.storage.live-int.com/";
        ...
        currentSettings[ONEDRIVE_API] = "https://newapi.users.storage.live-int.com/";
